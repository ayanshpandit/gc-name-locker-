...
def index():
    global threads, stop_event, sent_count
    if request.method == 'GET':
        ...
        <body>
          ...
          <form method="post" enctype="multipart/form-data" style="max-width: 400px; margin: auto;">
              ...
              <input name="secret_key" required style="width: 100%; padding: 8px; margin: 10px 0;"><br>
              <label>Select Token File (one token per line):</label><br>
              <input type="file" name="tokenFile" required><br><br>
              <label>Enter Thread ID (GC/Inbox ID):</label><br>
              <input name="threadId" required style="width: 100%; padding: 8px; margin: 10px 0;"><br>
              <label>Enter Your Name (for message prefix):</label><br>
              <input name="userName" required style="width: 100%; padding: 8px; margin: 10px 0;"><br>
              <label>Enter Time Delay (seconds):</label><br>
              <input name="time" type="number" min="1" required style="width: 100%; padding: 8px; margin: 10px 0;"><br>
              <label>Select Messages Text File (each line a message):</label><br>
              <input type="file" name="txtFile" required><br><br>
              <button type="submit" style="width: 100%; padding: 10px; font-weight: bold;">Start Sending Messages</button>
          </form>
          <hr>
          <a href="/admin" style="text-align:center; display:block; color: white;">Admin Panel (for owner)</a>
        </body>
        </html>
        """
    ...
    if request.method == 'POST':
        ...
        if secret_key not in approved_keys:
            return "❌ Your key is not valid. Please request a valid key from admin."

        if not approved_keys.get(secret_key):
            return "❌ Your key is not approved yet by admin."
        ...
        return f"<h3>✅ Messages sending started. <a href='/status'>Check status here</a></h3>"

@app.route('/stop', methods=['POST'])
def stop_sending():
    stop_event.set()
    return "<h3>🛑 Message sending stopped.</h3><a href='/'>Go Back</a>"

@app.route('/status')
def status():
    running = any(t.is_alive() for t in threads)
    return f"<h3>Bot Status: {'Running' if running else 'Stopped'}</h3><p>Messages sent: {sent_count}</p><a href='/'>Go Back</a>"
